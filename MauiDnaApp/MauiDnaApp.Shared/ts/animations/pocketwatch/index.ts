...
export class PocketWatchRenderer extends BaseRenderer {
    ...
    private drawOuterCase(ctx: CanvasRenderingContext2D) {
        ...
        this.drawScrewsOnRing(ctx, 355, 6, 6);
    }
    ...
    private drawMovement(ctx: CanvasRenderingContext2D, elapsed: number, totalTicks: number, tickCycle: number) {
        ...
        this.drawGear(ctx, -80, -90, 140, (elapsed / 60) * Math.PI * 2, '#424242', 40, true);
        this.drawRubyPin(ctx, -80, -90);

        this.drawGear(ctx, 110, 80, 100, -(elapsed / 15) * Math.PI * 2, '#8d6e63', 24, true);
        this.drawRubyPin(ctx, 110, 80);

        this.drawGear(ctx, 0, 0, 80, (elapsed / 60) * Math.PI * 6, '#bcaaa4', 20, false);
        this.drawRubyPin(ctx, 0, 0);
    ...
    private drawEscapementSystem(ctx: CanvasRenderingContext2D, x: number, y: number, elapsed: number, totalTicks: number, tickCycle: number) {
        ...
        this.drawEscapementWheel(ctx, 0, 0, 50, escapeRotation, '#ffd54f', teeth);
        this.drawRubyPin(ctx, 0, 0);
        ctx.restore();
    ...
    private drawEscapementWheel(ctx: CanvasRenderingContext2D, x: number, y: number, radius: number, angle: number, color: string, teeth: number) {
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(angle);
    ...
    private drawBridge(ctx: CanvasRenderingContext2D) {
        ...
        this.drawTinyScrew(ctx, 120, -100, 6);
        this.drawTinyScrew(ctx, 160, 60, 6);
        this.drawTinyScrew(ctx, 80, 140, 6);
        ...
        this.drawRubyPin(ctx, 0, 0);
        this.drawTinyScrew(ctx, -10, -10, 3);
        this.drawTinyScrew(ctx, 10, 10, 3);
        ctx.restore();
    ...
    private drawGear(ctx: CanvasRenderingContext2D, x: number, y: number, radius: number, angle: number, color: string, teeth: number, skeletize: boolean) {
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(angle);
    ...
    private drawDialRing(ctx: CanvasRenderingContext2D) {
        ...
        this.drawTinyScrew(ctx, -200, 0, 6);
        this.drawTinyScrew(ctx, 200, 0, 6);
        this.drawTinyScrew(ctx, 0, 200, 6);
    ...
    private drawScrewsOnRing(ctx: CanvasRenderingContext2D, radius: number, count: number, screwRadius: number) {
        for (let i = 0; i < count; i++) {
            const a = (i / count) * Math.PI * 2;
            const x = Math.cos(a) * radius;
            const y = Math.sin(a) * radius;
            this.drawTinyScrew(ctx, x, y, screwRadius);
        }
    }

    private drawTinyScrew(ctx: CanvasRenderingContext2D, x: number, y: number, r: number) {
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate((x + y) * 0.05);
    ...
    private drawRubyPin(ctx: CanvasRenderingContext2D, x: number, y: number) {
        ctx.save();
        ctx.translate(x, y);
...
export function startPocketWatchAnimation(canvasId: string) {
    new PocketWatchRenderer(canvasId).start();
}
